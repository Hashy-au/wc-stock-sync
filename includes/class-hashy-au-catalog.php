...
final class Hashy_AU_Catalog {
	...
	public function init(): void {
		...
		add_action( 'woocommerce_new_product', array( $this, 'invalidate_norm_map' ) );
		add_action( 'woocommerce_update_product', array( $this, 'invalidate_norm_map' ) );
		add_action( 'woocommerce_new_product_variation', array( $this, 'invalidate_norm_map' ) );
		add_action( 'woocommerce_update_product_variation', array( $this, 'invalidate_norm_map' ) );
		add_action( 'woocommerce_delete_product', array( $this, 'invalidate_norm_map' ) );
		add_action( 'woocommerce_trash_product', array( $this, 'invalidate_norm_map' ) );
	}
	...
	private function get_normalized_sku_map(): array {
		...
		if ( ! empty( $collisions ) && class_exists( 'Hashy_AU_Logger' ) ) {
			Hashy_AU_Logger::instance()->warning(
				'Normalized SKU collisions (lowest product ID wins)',
				array(
