...
final class Hashy_AU_Bootstrap {
	...
	public function init(): void {
		if ( ! class_exists( 'WooCommerce' ) ) {
			add_action( 'admin_notices', array( $this, 'notice_requires_woocommerce' ) );
			return;
		}
		...
		add_action( 'admin_init', array( $this, 'self_check_if_needed' ) );
		add_action( 'admin_notices', array( $this, 'maybe_show_self_check_notice' ) );
	...
	public function self_check_if_needed(): void {
		...
		$required_classes = array(
			'Hashy_AU_Settings' => array( 'instance', 'get_mode' ),
			'Hashy_AU_Mapping'  => array( 'instance', 'get_mappings' ),
			'Hashy_AU_Host'     => array( 'instance', 'init' ),
			'Hashy_AU_Agent'    => array( 'instance', 'init' ),
		);
		...
		if ( ! empty( $errors ) ) {
			...
			Hashy_AU_Logger::instance()->error( 'Self-check failed', array( 'errors' => $errors ) );
		} else {
