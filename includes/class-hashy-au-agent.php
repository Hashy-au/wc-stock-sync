...
final class Hashy_AU_Agent {
	...
	public function init(): void {
		...
		add_action( 'wcss_agent_process_outbox', array( $this, 'process_outbox' ) );
	...
	public function register_routes(): void {
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'rest_host_ping' ),
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'rest_host_stock_update' ),
				// Public by design: these calls carry no WordPress user. The
				// callback authenticates the request itself with
				// Hashy_AU_Crypto::verify() (HMAC-SHA256 over timestamp.body with
				// the shared secret) before doing any work, and answers a uniform
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'rest_host_sku_index' ),
				// Public by design: these calls carry no WordPress user. The
				// callback authenticates the request itself with
				// Hashy_AU_Crypto::verify() (HMAC-SHA256 over timestamp.body with
				// the shared secret) before doing any work, and answers a uniform
		...
		register_rest_route(
			$this->route_namespace,
			'/host/sku-index-detailed',
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'rest_host_sku_index_detailed' ),
				// Public by design: these calls carry no WordPress user. The
				// callback authenticates the request itself with
				// Hashy_AU_Crypto::verify() (HMAC-SHA256 over timestamp.body with
				// the shared secret) before doing any work, and answers a uniform
	...
	public function on_status_changed( $order_id, $old_status, $new_status, $order ): void {
		...
		if ( ! in_array( (string) $new_status, array( 'processing', 'completed' ), true ) ) {
			return;
		}
	...
	private function send_order_paid( int $order_id ): void {
		...
		if ( empty( $host_url ) || empty( $secret ) ) {
			Hashy_AU_Logger::instance()->warning( 'Agent not configured (host_url/shared_secret missing)', array( 'order_id' => $order_id ) );
			return;
		}
	...
	public function send_test_order_paid_by_sku( string $sku, int $qty = 1 ): array {
		...
		$secret   = Hashy_AU_Settings::instance()->get_agent_shared_secret();
		if ( empty( $host_url ) || empty( $secret ) ) {
			Hashy_AU_Logger::instance()->warning( 'Agent test order-paid not configured (host_url/shared_secret missing)', array( 'sku' => $sku ) );
			return array(
				'ok'    => false,
		...
		Hashy_AU_Logger::instance()->info(
			'Test order-paid notify response',
			array(
				'sku'  => $sku,
				'code' => $code,
	...
	public function rest_host_sku_index( WP_REST_Request $request ): WP_REST_Response {
		...
		$secret = Hashy_AU_Settings::instance()->get_agent_shared_secret();
		if ( empty( $secret ) || ! Hashy_AU_Crypto::verify( $secret, $timestamp, $raw_body, $signature ) ) {
			Hashy_AU_Logger::instance()->warning( 'Bad signature on sku-index', array() );
			return new WP_REST_Response(
				array(
					'ok'    => false,
					'error' => 'bad_signature',
	...
	public function rest_host_sku_index_detailed( WP_REST_Request $request ): WP_REST_Response {
		...
		$secret = Hashy_AU_Settings::instance()->get_agent_shared_secret();
		if ( empty( $secret ) || ! Hashy_AU_Crypto::verify( $secret, $timestamp, $raw_body, $signature ) ) {
			Hashy_AU_Logger::instance()->warning( 'Bad signature on sku-index-detailed', array() );
			return new WP_REST_Response(
				array(
					'ok'    => false,
					'error' => 'bad_signature',
		...
		return new WP_REST_Response(
			array(
				'ok'    => true,
				'items' => Hashy_AU_Catalog::instance()->get_local_sku_items(),
			),
			200
		);
	}
	...
	public function rest_host_stock_update( WP_REST_Request $request ): WP_REST_Response {
		...
		if ( ! Hashy_AU_Crypto::verify( $secret, $timestamp, $raw_body, $signature ) ) {
			Hashy_AU_Logger::instance()->warning( 'Bad signature on stock update', array() );
			return new WP_REST_Response(
				array(
					'ok'    => false,
					'error' => 'bad_signature',
		...
		if ( ! $product_id ) {
			$this->record_missing( array( $sku ) );
			Hashy_AU_Logger::instance()->warning( 'SKU not found on agent', array( 'sku' => $sku ) );
			return new WP_REST_Response(
				array(
					'ok'    => false,
					'error' => 'sku_not_found',
		...
		foreach ( array( 'regular_price', 'sale_price' ) as $p ) {
	...
	public function daily_reconcile(): void {
		Hashy_AU_Logger::instance()->info( 'Daily reconcile ran (agent)', array() );
	}
	...
	private function enqueue_outbox( array $entry ): void {
		$outbox = get_option( 'wcss_agent_outbox', array() );
		if ( ! is_array( $outbox ) ) {
			$outbox = array();
		}
	...
	public function process_outbox(): void {
		...
		foreach ( $outbox as $entry ) {
			...
			if ( $now - $created_at <= DAY_IN_SECONDS ) {
				...
				$dead = get_option( 'wcss_agent_outbox_dead', array() );
				if ( ! is_array( $dead ) ) {
					$dead = array();
				}
				$dead[] = $entry;
				...
				Hashy_AU_Logger::instance()->error( 'Outbox dropped after 24h', array( 'order_id' => $order_id ) );
			}
		}
