...
final class Hashy_AU_Mapping {
	...
	public function init(): void {
		add_action( 'admin_post_hashy_au_export_skus', array( $this, 'export_skus' ) );
		add_action( 'admin_post_wcss_export_all_skus', array( $this, 'export_all_skus' ) );
		add_action( 'admin_post_wcss_export_synced_skus', array( $this, 'export_synced_skus' ) );
		add_action( 'admin_post_wcss_export_local_skus', array( $this, 'export_local_skus' ) );
		add_action( 'admin_post_wcss_import_sku_mappings_draft', array( $this, 'import_sku_mappings_draft' ) );
		add_action( 'admin_post_wcss_import_sku_mappings_apply', array( $this, 'import_sku_mappings_apply' ) );
		add_action( 'admin_post_hashy_au_export_mappings', array( $this, 'export_mappings' ) );

		// Back-compat (if triggered via admin-ajax).
		add_action( 'wp_ajax_hashy_au_export_skus', array( $this, 'export_skus' ) );
		add_action( 'wp_ajax_hashy_au_export_mappings', array( $this, 'export_mappings' ) );
	}
	...
	public function get_mappings(): array {
		$m = get_option( $this->option_name, array() );
	...
	public function get_agent_overrides(): array {
		$m = get_option( $this->agent_overrides_option, array() );
		return is_array( $m ) ? $m : array();
	}
	...
	public function export_all_skus(): void {
		...
		$selected = isset( $_GET['agents'] ) ? array_map( 'sanitize_key', (array) wp_unslash( $_GET['agents'] ) ) : array();
		...
		$headers = array( 'normalized_key', 'match_source', 'host_product_name', 'host_variation_name', 'host_sku', 'host_type' );
	...
	public function export_synced_skus(): void {
		...
		$groups = array_values(
			array_filter(
				$groups,
				function ( $g ) use ( $agent_rows ) {
					if ( empty( $g['host_sku'] ) ) {
						return false;
					}
					foreach ( $agent_rows as $k => $_ ) {
						if ( ! empty( $g['agents'][ $k ]['sku'] ) ) {
							return true;
						}
					}
					return false;
	...
	private function build_normalized_groups( array $host_items, array $agent_rows ): array {
		...
		foreach ( $agent_rows as $agent_key => $info ) {
			...
			foreach ( $items as $it ) {
				...
				$groups[ $key ]['agents'][ $agent_key ] = array(
					'sku'            => (string) ( $it['sku'] ?? '' ),
	...
	public function export_local_skus(): void {
		...
		$out = fopen( 'php://output', 'w' ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen -- php://output stream for the CSV download; WP_Filesystem does not apply.
		$this->write_csv_row( $out, array( 'product_name', 'variation_name', 'sku', 'type', 'normalized_key' ) );
	...
	function import_sku_mappings_draft(): void {
		...
		foreach ( $changes as $c ) {
			...
			if ( isset( $seen[ $k ] ) && $seen[ $k ] !== $c['host_sku'] ) {
				$warnings[] = array(
					'type'       => 'conflict_duplicate_agent_sku',
					'agent'      => $c['agent'],
					'agent_sku'  => $c['agent_sku'],
					'host_sku_a' => $seen[ $k ],
					'host_sku_b' => $c['host_sku'],
				);
			} else {
				$seen[ $k ] = $c['host_sku'];
			}
	...
	public function import_sku_mappings_apply(): void {
		...
		Hashy_AU_Logger::instance()->info(
			...
			array(
				'changes'  => count( $draft['changes'] ),
				'warnings' => count( (array) ( $draft['warnings'] ?? array() ) ),
			)
		);
	...
	public function export_skus(): void {
		...
		$out = fopen( 'php://output', 'w' ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen -- php://output stream for the CSV download; WP_Filesystem does not apply.
		$this->write_csv_row( $out, array( 'Product Name', 'Variation Name', 'SKU', 'Product ID', 'Variation ID' ) );
		...
		foreach ( $products as $product ) {
			...
			$sku = (string) $product->get_sku();
			$this->write_csv_row( $out, array( $product->get_name(), '', $sku, $product->get_id(), '' ) );
		}
	...
	public function export_mappings(): void {
		...
		$out = fopen( 'php://output', 'w' ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen -- php://output stream for the CSV download; WP_Filesystem does not apply.
		$this->write_csv_row( $out, array( 'normalized_sku', 'host_sku' ) );

		$m = $this->get_mappings();
		foreach ( $m as $k => $v ) {
			$this->write_csv_row( $out, array( $k, $v ) );
		}
	...
	private function write_csv_row( $handle, array $row ): void {
		fputcsv( $handle, array_map( array( Hashy_AU_Catalog::class, 'csv_safe' ), $row ), ',', '"', '' );
	}
}
