...
final class Hashy_AU_Settings {
	...
	public function init(): void {
		...
		add_action( 'wp_ajax_wcss_generate_secret', array( $this, 'ajax_generate_secret' ) );
		add_action( 'wp_ajax_wcss_test_agent', array( $this, 'ajax_test_agent' ) );
		add_action( 'wp_ajax_wcss_test_host', array( $this, 'ajax_test_host' ) );
		add_action( 'wp_ajax_wcss_send_test_order_paid', array( $this, 'ajax_send_test_order_paid' ) );
		add_action( 'wp_ajax_wcss_start_price_sync', array( $this, 'ajax_start_price_sync' ) );
		add_action( 'wp_ajax_wcss_run_price_sync_batch', array( $this, 'ajax_run_price_sync_batch' ) );
		add_action( 'wp_ajax_wcss_start_stock_sync', array( $this, 'ajax_start_stock_sync' ) );
		add_action( 'wp_ajax_wcss_run_stock_sync_batch', array( $this, 'ajax_run_stock_sync_batch' ) );
		add_action( 'wp_ajax_wcss_clear_logs', array( $this, 'ajax_clear_logs' ) );
	}
	...
	public function get_all(): array {
		$defaults = array(
			...
			'normalize_skus' => 'yes',
			'host'           => array(
				'agents' => array(), // array: id, name, url, shared_secret, price_pct, sync_prices
			),
			'agent'          => array(
				'host_url'      => '',
				'agent_code'    => '',
	...
	public function get_mode(): string {
		...
		return in_array( $mode, array( 'host', 'agent' ), true ) ? $mode : 'agent';
	}
	...
	public function register_admin_menu(): void {
		...
		add_submenu_page( 'wcss', 'Settings', 'Settings', 'manage_woocommerce', 'wcss', array( $this, 'render_settings_page' ) );
		add_submenu_page( 'wcss', 'Import/Export', 'Import/Export', 'manage_woocommerce', 'wcss-import-export', array( $this, 'render_import_export_page' ) );
		add_submenu_page( 'wcss', 'Missing SKUs', 'Missing SKUs', 'manage_woocommerce', 'wcss-missing-skus', array( $this, 'render_missing_skus_page' ) );
		add_submenu_page( 'wcss', 'Logs', 'Logs', 'manage_woocommerce', 'wcss-logs', array( $this, 'render_logs_page' ) );
	}

	public function register_settings(): void {
		register_setting( 'wcss_settings', $this->option_name, array( $this, 'sanitize_settings' ) );
	}
	...
	public function sanitize_settings( $input ): array {
		...
		$out['mode'] = in_array( $out['mode'], array( 'host', 'agent' ), true ) ? $out['mode'] : 'agent';
		...
		if ( isset( $input['host'] ) && is_array( $input['host'] ) ) {
			$agents        = $input['host']['agents'] ?? array();
			$clean_agents  = array();
			$agent_secrets = array();
			...
			<form method="post" action="options.php">
				...
				<?php else : ?>
					...
					<table class="widefat striped" id="wcss_agents_table">
						...
						<tbody>
						<?php
						$agents = $settings['host']['agents'] ?? array();
						if ( ! is_array( $agents ) ) {
							$agents = array();
						}
	...
	public function render_import_export_page(): void {
		...
		<div class="wrap">
			...
			<?php else : ?>
				...
				<?php if ( $show_draft ) : ?>
					<?php
					$changes       = is_array( $draft['changes'] ?? null ) ? $draft['changes'] : array();
					$warnings      = is_array( $draft['warnings'] ?? null ) ? $draft['warnings'] : array();
					...
					$toggle_url    = add_query_arg(
						array(
							...
							'wcss_draft'         => '1',
							'wcss_only_warnings' => $only_warnings ? '0' : '1',
						),
						admin_url( 'admin.php' )
					);
					?>
...
(function(){
	...
	public function ajax_test_agent(): void {
		...
		if ( $this->get_mode() !== 'host' ) {
			wp_send_json_error( array( 'message' => 'not_host' ), 400 );
		}
		...
		if ( empty( $agent_url ) || empty( $secret ) ) {
			wp_send_json_error( array( 'message' => 'agent_url/secret missing' ), 400 );
		}
		...
		if ( 'https' !== wp_parse_url( $agent_url, PHP_URL_SCHEME ) ) {
			wp_send_json_error( array( 'message' => 'Agent URL must use https' ), 400 );
		}
	...
	public function ajax_test_host(): void {
		...
		if ( $this->get_mode() !== 'agent' ) {
			wp_send_json_error( array( 'message' => 'not_agent' ), 400 );
		}
		...
		if ( empty( $host_url ) || empty( $secret ) ) {
			wp_send_json_error( array( 'message' => 'host_url/shared_secret missing' ), 400 );
		}
		...
		if ( is_wp_error( $res ) ) {
			Hashy_AU_Logger::instance()->error( 'Test host failed (wp_error)', array( 'error' => $res->get_error_message() ) );
		...
		if ( $code < 200 || $code >= 300 ) {
			...
			wp_send_json_error( array( 'message' => 'HTTP ' . $code . ': ' . substr( $resp_body, 0, 200 ) ), 500 );
		}

		Hashy_AU_Logger::instance()->info( 'Test host OK', array( 'code' => $code ) );
		wp_send_json_success(
			array(
				'ok'   => true,
				'code' => $code,
	...
	public function ajax_send_test_order_paid(): void {
		...
		if ( empty( $sku ) ) {
			wp_send_json_error( array( 'message' => 'missing_sku' ), 400 );
		}
	...
	public function ajax_start_price_sync(): void {
		...
		if ( 'host' !== $this->get_mode() ) {
			wp_send_json_error( array( 'message' => 'Host mode only' ), 400 );
		}

		$idx    = isset( $_POST['agent_index'] ) ? (int) $_POST['agent_index'] : -1;
		$agents = $this->get_host_agents();
		if ( ! isset( $agents[ $idx ] ) || ! is_array( $agents[ $idx ] ) ) {
			wp_send_json_error( array( 'message' => 'Unknown agent index' ), 400 );
		}

		$agent = $agents[ $idx ];
		if ( empty( $agent['url'] ) || empty( $agent['shared_secret'] ) ) {
			wp_send_json_error( array( 'message' => 'Agent URL/secret missing' ), 400 );
		}

		if ( 'yes' !== (string) ( $agent['sync_prices'] ?? 'no' ) ) {
			wp_send_json_error( array( 'message' => 'Enable “Sync Prices” for this agent first.' ), 400 );
		}

		Hashy_AU_Logger::instance()->info( 'Price sync started (admin)', array( 'agent_url' => (string) $agent['url'] ) );
		$res = Hashy_AU_Host::instance()->process_price_sync_batch_once( $agent, 1 );
		if ( ! empty( $res['error'] ) ) {
			wp_send_json_error( array( 'message' => (string) $res['error'] ), 400 );
		}
		wp_send_json_success( $res );
	}
	...
	public function ajax_start_stock_sync(): void {
		...
		Hashy_AU_Logger::instance()->info( 'Stock sync started (admin)', array( 'agent_url' => (string) $agent['url'] ) );
