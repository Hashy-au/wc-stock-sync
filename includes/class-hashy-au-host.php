...
final class Hashy_AU_Host {
	 ...
	 * @var array<int, bool>
	 */
	private static array $pushed_ids = array();
	...
	public function init(): void {
		...
		// Retry queue.
		add_action( 'wcss_retry_failed_requests', array( $this, 'process_failed_queue' ) );
	...
	public function rest_host_ping( WP_REST_Request $request ): WP_REST_Response {
		...
		Hashy_AU_Logger::instance()->info( 'Host ping OK', array( 'agent_url' => $agent_url ) );
	...
	private function log_auth_failure( string $context ): void {
		...
		Hashy_AU_Logger::instance()->warning( 'Rejected unauthenticated sync request(s)', array( 'context' => $context ) );
	}
	...
	public function rest_agent_order_paid( WP_REST_Request $request ): WP_REST_Response {
		...
		Hashy_AU_Logger::instance()->info(
			'Inbound agent order-paid received',
			array(
				'agent_url'   => $agent_url,
				'order_id'    => $order_id,
	...
	private function maybe_push_product( WC_Product $product ): void {
		...
		if ( $product->is_type( 'variation' ) && '' === (string) $product->get_sku( 'edit' ) ) {
			Hashy_AU_Logger::instance()->info( 'Skipping push for variation without own SKU', array( 'product_id' => $pid ) );
			return;
		}
	...
	public function process_price_sync_batch_once( array $agent, int $page ): array {
		...
		if ( null === $agent_skus ) {
			return array(
				...
				'next_page' => $page,
				'processed' => 0,
				'error'     => 'Could not fetch the agent SKU index: abortingprice sync. See Logs.',
			);
		}
		if ( empty( $agent_skus ) ) {
			return array(
				...
				'next_page' => $page,
				'processed' => 0,
				'error'     => 'Agent reports no SKUs: abortingprice sync.',
			);
		}
		...
		if ( empty( $ids ) ) {
			Hashy_AU_Logger::instance()->info( 'Price sync finished', array( 'agent_url' => $agent_url ) );
			return array(
				'done'      => true,
				'next_page' => $page,
				'processed' => 0,
	...
	public function process_stock_sync_batch_once( array $agent, int $page ): array {
		...
		if ( null === $agent_skus ) {
			return array(
				...
				'next_page' => $page,
				'processed' => 0,
				'error'     => 'Could not fetch the agent SKU index: abortingstock sync. See Logs.',
			);
		}
		if ( empty( $agent_skus ) ) {
			return array(
				...
				'next_page' => $page,
				'processed' => 0,
				'error'     => 'Agent reports no SKUs: abortingstock sync.',
			);
		}
		...
		if ( empty( $ids ) ) {
			Hashy_AU_Logger::instance()->info( 'Stock sync finished', array( 'agent_url' => $agent_url ) );
			return array(
				'done'      => true,
				'next_page' => $page,
				'processed' => 0,
	...
	public function process_failed_queue(): void {
		...
		foreach ( $queue as $row ) {
			...
			if ( ! $agent || empty( $payload ) ) {
				Hashy_AU_Logger::instance()->warning(
					'Dropping queued push (agent no longer configured)',
					array(
			...
			if ( $row['attempts'] >= $max_attempts ) {
				Hashy_AU_Logger::instance()->warning(
					'Dropping queued push after max attempts',
					array(
						'agent_url' => (string) ( $row['agent_url'] ?? '' ),
						'sku'       => (string) ( $payload['sku'] ?? '' ),
					)
	...
	private function fetch_agent_normalized_skus( array $agent ): ?array {
		...
		$rows = is_array( $data ) ? ( $data['skus'] ?? array() ) : array();
		if ( ! is_array( $rows ) ) {
			return null;
		}
	...
	public function fetch_agent_sku_items( array $agent ): array {
		...
		if ( is_wp_error( $res ) ) {
			Hashy_AU_Logger::instance()->warning(
				'Agent sku-index-detailed failed',
				array(
					'agent_url' => $agent_url,
					'error'     => $res->get_error_message(),
				)
		...
		if ( $code < 200 || $code >= 300 ) {
			Hashy_AU_Logger::instance()->warning(
				'Agent sku-index-detailed non-2xx',
				array(
					'agent_url' => $agent_url,
					'code'      => $code,
