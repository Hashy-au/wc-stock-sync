...
final class Hashy_AU_Stocktake {
	...
	public function init(): void {
		add_action( 'admin_post_wcss_stocktake_export', array( $this, 'handle_export' ) );
		add_action( 'admin_post_wcss_stocktake_draft', array( $this, 'handle_draft' ) );
		add_action( 'wp_ajax_wcss_stocktake_apply_start', array( $this, 'ajax_apply_start' ) );
		add_action( 'wp_ajax_wcss_stocktake_apply_batch', array( $this, 'ajax_apply_batch' ) );
		add_action( 'wp_ajax_wcss_stocktake_push_batch', array( $this, 'ajax_push_batch' ) );
		add_action( self::CRON_HOOK, array( $this, 'cron_drain' ) );
	}
	...
	public function handle_export(): void {
		...
		$sheet = array( array( 'Name', 'SKU', 'Stock', 'New Stock', 'Product ID (do not edit)' ) );
	...
	public function handle_draft(): void {
		...
		set_transient(
			self::DRAFT_TRANSIENT_PREFIX . get_current_user_id(),
			array(
				'ts'       => time(),
				'changes'  => $changes,
				'warnings' => $warnings,
		...
		if ( $pid > 0 ) {
			...
			if ( '' !== $sku && (string) $product->get_sku( 'edit' ) !== $sku ) {
				$warnings[] = array(
					...
					'sku'    => $sku,
					'code'   => 'sku_mismatch',
					'detail' => (string) $product->get_sku( 'edit' ),
				);
				return null;
			}
			return $product;
	...
	public function ajax_apply_start(): void {
		...
		if ( ! $draft || empty( $draft['changes'] ) ) {
			wp_send_json_error( array( 'message' => 'No stocktake draft to apply. Upload a file first.' ), 400 );
		}
		...
		Hashy_AU_Logger::instance()->info( 'Stocktake apply started', array( 'rows' => count( $draft['changes'] ) ) );
		wp_send_json_success( array( 'total' => count( $draft['changes'] ) ) );
	}
	...
	public function ajax_apply_batch(): void {
		...
		if ( ! is_array( $job ) || empty( $job['rows'] ) ) {
			wp_send_json_error( array( 'message' => 'No stocktake apply in progress.' ), 400 );
		}
		...
		foreach ( $slice as $row ) {
			$result = $this->apply_row( is_array( $row ) ? $row : array() );
			if ( true === $result ) {
	...
	private function queue_pushes( array $product_ids ): void {
		...
		$queue = get_option( self::PUSH_QUEUE_OPTION, array() );
		if ( ! is_array( $queue ) ) {
			$queue = array();
		}
	...
	public function render_admin_section(): void {
		...
		<?php if ( $show_draft ) : ?>
			...
			$warnings = (array) ( $draft['warnings'] ?? array() );
			$counts   = (array) ( $draft['counts'] ?? array() );
			?>
			<h3>Preview</h3>
			<p>
