...
final class Hashy_AU_Secrets {
	...
	private static function read_store(): array {
		...
		$agents = ( isset( $raw['agents'] ) && is_array( $raw['agents'] ) ) ? $raw['agents'] : array();
	...
	private static function migrate_legacy(): void {
		$legacy = get_option( self::SETTINGS_OPTION, array() );
		if ( ! is_array( $legacy ) ) {
			return;
		}
		...
		if ( isset( $legacy['agent'] ) && is_array( $legacy['agent'] ) ) {
			foreach ( array( 'shared_secret', 'host_shared_secret' ) as $field ) {
		...
		if ( class_exists( 'Hashy_AU_Logger' ) ) {
			Hashy_AU_Logger::instance()->info(
				'Secrets moved out of hashy_au_settings into hashy_au_secrets (autoload off)',
				array(
	...
	private static function normalise( array $store ): array {
		$agents     = array();
		$raw_agents = ( isset( $store['agents'] ) && is_array( $store['agents'] ) ) ? $store['agents'] : array();
